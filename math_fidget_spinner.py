...
with BuildPart() as fidget_spinner_button_top:
    ...
    with BuildSketch(top_face) as s:
        text_baseline_radius = (8 - tolerance) / 2
        Circle(text_baseline_radius)
    extrude(amount=4)
    chamfer(
        fidget_spinner_button_top.edges()
        .filter_by(GeomType.CIRCLE)
...
# %%
show(pack([fidget_spinner_button_top.part, fidget_spinner_disc.part], 5))
